...
CREATE FUNCTION app.claim_drive_sync_job(
...
BEGIN
    ...
    WHERE exhausted.id = attempt.job_id
      ...
      AND (exhausted.payload -> 'source_families') ? 'google_drive'
    ...
    WHERE candidate.job_type = 'sync'
      ...
      AND (candidate.payload -> 'source_families') ? 'google_drive'
...
ALTER FUNCTION app.claim_drive_sync_job(TEXT, INTEGER) OWNER TO app_migrator;
REVOKE ALL ON FUNCTION app.claim_drive_sync_job(TEXT, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.claim_drive_sync_job(TEXT, INTEGER) TO app_worker;
