...
**Audited baseline:** `1b0bead`; this ledger includes the current Drive DOCX and
native Google Docs extraction checkpoint within `B7`.
...
| `06_CONNECTOR_FRAMEWORK.md` | Complete | SDK, OAuth, Gmail backend phase, Drive folder traversal, and PDF/DOCX/Google Docs extraction implemented; provider ecosystem partial | The complete tested Gmail backend and bounded read-only Drive traversal plus PDF, uploaded DOCX, and native Google Docs extraction are implemented. Drive uses durable jobs, encrypted traversal progress, stable identity/metadata, shared-drive parameters, shortcut non-traversal, bounded media/export downloads, structure-aware parsers, and safe fallback states. Sheets, Slides, deletion reconciliation, GitHub, local files, general scheduling, logging, and metrics remain. |
| `07_INDEXING_PIPELINE.md` | Complete | Normalized text/Markdown plus Drive PDF/DOCX parser paths implemented | Deterministic normalization, structure-aware bounded PDF/DOCX extraction, language selection, chunking, exact/near deduplication, local 1,536-dimensional embeddings, durable PostgreSQL leasing, atomic promotion, unchanged skips, and re-index supersession pass unit and PostgreSQL/pgvector tests. Remaining provider-specific parsers and production embedding providers remain later integrations. |
...
derived-data deletion, and error recovery. Bounded Drive folder traversal plus PDF,
DOCX, and native Google Docs extraction are complete; Google Sheets is next.
...
| `GOOGLE-001` | Partial | The Google authorization endpoints implement workspace/session binding, PKCE S256, exact Gmail/Drive read-only scopes, encrypted credentials, and per-family jobs. The Gmail backend phase is complete. Drive traverses selected roots through bounded durable jobs, supports shared drives without following shortcuts, extracts bounded PDF/DOCX text, and exports native Google Docs into that DOCX path while retaining provider identity. | Sheets/Slides parsers, selection/connection/search UI, incremental Drive changes/deletion, and a live Google sandbox smoke test. |
...
- `150 backend tests at 90.74% line coverage`, `20 PostgreSQL integration tests`,
...
- The live PostgreSQL traversal test proves PDF, uploaded DOCX, and exported
  Google Docs text all reach ready searchable document versions.

Drive master tasks complete: `6/10`: `P6-001` through `P6-006`. Google Sheets
export/extraction (`P6-007`) is next, followed by Slides, delete handling, and
the Drive phase review.
...
- Google Sheets/Slides format parsers, incremental Drive changes, and deletion
  handling; bounded folder traversal and PDF/DOCX/Google Docs extraction are
...
| `B7` | Real provider and desktop slices | **In progress:** The Gmail backend phase plus bounded Drive traversal and PDF/DOCX/Google Docs extraction are complete. Sheets and Slides are next, followed by incremental deletion, GitHub, and local desktop behavior, each certified and integrated end to end. |
